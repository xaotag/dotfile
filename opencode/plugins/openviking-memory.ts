...
function serializeSessionMapping(
...
function deserializeSessionMapping(
  persisted: SessionMappingPersisted,
): SessionMapping {
  return {
...
async function loadSessionMap(): Promise<void> {
  ...
  try {
    ...
    if (data.version !== 1) {
      log("ERROR", "persistence", "Unsupported session map version", {
    ...
    for (
      const [opencodeSessionId, persisted] of Object.entries(data.sessions)
    ) {
    ...
    if (fs.existsSync(sessionMapPath)) {
      ...
      log("INFO", "persistence", "Corrupted file backed up", {
        backup: backupPath,
      });
    }
  }
...
async function saveSessionMap(): Promise<void> {
  ...
  try {
    ...
    log("DEBUG", "persistence", "Session map saved", {
      count: sessionMap.size,
    });
  } catch (error: any) {
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (config.autoCommit) {
        config.autoCommit.intervalMinutes = getAutoCommitIntervalMinutes(
          config,
        );
      }
...
async function makeRequest<T = any>(
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        `Request timeout after ${options.timeoutMs ?? config.timeoutMs}ms`,
      );
    }
...
function upsertBufferedMessage(
  ...
  if (now - lastBufferCleanupAt >= BUFFER_CLEANUP_INTERVAL_MS) {
    for (
      const [bufferedSessionId, bufferedMessages] of sessionMessageBuffer
        .entries()
    ) {
      const freshMessages = bufferedMessages.filter((message) =>
        now - message.timestamp <= BUFFERED_MESSAGE_TTL_MS
      );
  ...
  const freshBuffer = existingBuffer.filter((message) =>
    now - message.timestamp <= BUFFERED_MESSAGE_TTL_MS
  );
...
function getAutoCommitIntervalMinutes(config: OpenVikingConfig): number {
  const configured = Number(
    config.autoCommit?.intervalMinutes ??
      DEFAULT_CONFIG.autoCommit?.intervalMinutes ?? 10,
  );
...
async function ensureOpenVikingSession(
  ...
  if (knownSessionId) {
    try {
      const response = await makeRequest<OpenVikingResponse<SessionResult>>(
        config,
        {
          method: "GET",
      ...
      log(
        "INFO",
        "session",
        "Persisted OpenViking session unavailable, creating a new one",
        {
          opencode_session: opencodeSessionId,
          openviking_session: knownSessionId,
          error: error.message,
  ...
  try {
    const createResponse = await makeRequest<OpenVikingResponse<SessionResult>>(
      config,
      {
        method: "POST",
...
async function findRunningCommitTaskId(
  ...
  try {
    const response = await makeRequest<OpenVikingResponse<TaskResult[]>>(
      config,
      {
        method: "GET",
        endpoint: `/api/v1/tasks?task_type=session_commit&resource_id=${
          encodeURIComponent(ovSessionId)
        }&limit=10`,
        timeoutMs: 5000,
      },
    );
...
async function detectBackgroundCommitSupport(
  config: OpenVikingConfig,
): Promise<boolean> {
...
async function finalizeCommitSuccess(
  ...
  if (mapping.pendingCleanup) {
    ...
    log(
      "INFO",
      "session",
      "Cleaned up session mapping after commit completion",
      {
...
async function runSynchronousCommit(
  ...
  try {
    const response = await makeRequest<OpenVikingResponse<CommitResult>>(
      config,
      {
        method: "POST",
...
async function flushPendingMessages(
  ...
  for (const messageId of Array.from(mapping.pendingMessages.keys())) {
    if (
      mapping.capturedMessages.has(messageId) ||
      mapping.sendingMessages.has(messageId)
    ) {
      continue;
    }
      ...
      if (success) {
        ...
        if (latestContent && latestContent !== content) {
          log(
            "DEBUG",
            "message",
            "Message changed during send; keeping latest content pending",
            {
              session_id: opencodeSessionId,
              message_id: messageId,
              role,
...
async function startBackgroundCommit(
  ...
  if (!supportsBackgroundCommit) {
    try {
      const result = await runSynchronousCommit(
        mapping,
        opencodeSessionId,
        config,
        abortSignal,
...
async function checkAndCommitSessions(config: OpenVikingConfig): Promise<void> {
  ...
  for (const [opencodeSessionId, mapping] of sessionMap.entries()) {
    ...
    const timeSinceLastCommit = now -
      (mapping.lastCommitTime ?? mapping.createdAt);
...
function validateVikingUri(uri: string, toolName: string): string | null {
  if (!uri.startsWith("viking://")) {
    const error =
      `Invalid URI format. Must start with "viking://". Example: viking://user/memories/`;
...
export const OpenVikingMemoryPlugin = async (
  input: PluginInput,
): Promise<Hooks> => {
  ...
  log("INFO", "plugin", "OpenViking Memory Plugin initialized", {
    endpoint: config.endpoint,
  });
  ...
  log(
    ...
    "health",
    healthy
      ? "OpenViking health check passed"
            ...
            for (const buffered of bufferedMessages) {
              ...
              if (buffered.content) {
                mapping.pendingMessages.set(
                  buffered.messageId,
                  mergeMessageContent(
                    mapping.pendingMessages.get(buffered.messageId),
                    buffered.content,
                  ),
                );
              }
        ...
        if (!mapping) {
          ...
          if (partType === "text" && part.text && part.text.trim().length > 0) {
            ...
            log(
              "DEBUG",
              "message",
              "Message content buffered (no session mapping yet)",
              {
                session_id: sessionId,
                message_id: messageId,
        ...
        if (partType === "text" && part.text) {
          ...
          if (content && content.trim().length > 0) {
            mapping.pendingMessages.set(
              messageId,
              mergeMessageContent(
                mapping.pendingMessages.get(messageId),
                content,
              ),
            );
      ...
      memread: tool({
        ...
        args: {
          ...
          level: z
            ...
            .optional()
            .describe(
              "'auto' (directory->overview, file->read), 'abstract' (brief summary), 'overview' (directory summary), 'read' (full content)",
            ),
        },
        async execute(args, context) {
          log("INFO", "memread", "Reading memory", {
            uri: args.uri,
            level: args.level,
          });
          ...
          try {
            ...
            if (level === "auto") {
              try {
                const statResponse = await makeRequest<
                  OpenVikingResponse<{ isDir?: boolean }>
                >(config, {
                  method: "GET",
                  endpoint: `/api/v1/fs/stat?uri=${
            ...
            const response = await makeRequest<
              OpenVikingResponse<string | Record<string, unknown>>
            >(config, {
              method: "GET",
              endpoint: `/api/v1/content/${level}?uri=${
                encodeURIComponent(args.uri)
              }`,
              abortSignal: context.abort,
            });
      ...
      membrowse: tool({
        ...
        args: {
          ...
          view: z
            ...
            .optional()
            .describe(
              "'list' for directory listing, 'tree' for recursive tree view, 'stat' for metadata on a single URI",
            ),
          recursive: z.boolean().optional().describe(
            "Only used with view='list'. Recursively list descendants.",
          ),
          simple: z.boolean().optional().describe(
            "Only used with view='list'. Return simpler URI-oriented output.",
          ),
        },
        async execute(args, context) {
          ...
          try {
            ...
            if (view === "stat") {
              const response = await makeRequest<
                OpenVikingResponse<Record<string, unknown>>
              >(config, {
                method: "GET",
            ...
            const endpoint = view === "tree"
              ...
              : `/api/v1/fs/ls?uri=${encodedUri}&recursive=${
                args.recursive ? "true" : "false"
              }&simple=${args.simple ? "true" : "false"}`;
            const response = await makeRequest<OpenVikingResponse<any[]>>(
              config,
              {
                method: "GET",
                endpoint,
            ...
            return JSON.stringify(
              { view, count: items.length, items },
              null,
              2,
            );
          } catch (error: any) {
            log("ERROR", "membrowse", "Browse failed", {
              error: error.message,
              uri: args.uri,
            });
            return `Error: ${error.message}`;
      ...
      memcommit: tool({
        ...
        args: {
          session_id: z
            ...
            .optional()
            .describe(
              "Optional explicit OpenViking session ID. Omit to commit the current OpenCode session's mapped OpenViking session.",
            ),
        },
        async execute(args, context) {
          ...
          try {
            const mapping = context.sessionID
              ? sessionMap.get(context.sessionID)
              : undefined;
            const resolvedMapping = mapping?.ovSessionId === sessionId
              ? mapping
              : undefined;
            ...
            if (resolvedMapping?.commitInFlight) {
              ...
              if (task?.status === "completed") {
                return JSON.stringify(
                  {
                    message: `Memory extraction complete: ${
                      task.result?.memories_extracted ?? 0
                    } memories extracted`,
            ...
            if (commitStart.mode === "completed") {
              return JSON.stringify(
                {
                  message: `Memory extraction complete: ${
                    commitStart.result.memories_extracted ?? 0
                  } memories extracted`,
                  session_id: commitStart.result.session_id ?? sessionId,
                  status: commitStart.result.status ?? "completed",
                  memories_extracted: commitStart.result.memories_extracted ??
                    0,
      ...
      memsearch: tool(
        {
          ...
          args: {
            query: z.string().describe(
              "Search query - can be natural language, a complex question, or a task description",
            ),
            target_uri: z
              .string()
              .optional()
            ...
            mode: z
              ...
              .optional()
              .describe(
                "Search mode. 'auto' chooses based on query complexity and session context, 'fast' forces /find, 'deep' forces /search",
              ),
            session_id: z
              .string()
              .optional()
          ...
          async execute(args, context) {
            ...
            if (args.score_threshold !== undefined) {
              requestBody.score_threshold = args.score_threshold;
            }
            ...
            try {
              const response = await makeRequest<
                OpenVikingResponse<SearchResult>
              >(config, {
                method: "POST",
                endpoint: mode === "deep"
              ...
              log("ERROR", "memsearch", "Search failed", {
                error: error.message,
                args,
              });
              return `Error: ${error.message}`;
